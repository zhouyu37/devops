    ...
    def get_list_filter(self, list_filter):
        ...
        for i in list_filter:
            ...
            if col_obj.get_internal_type() not in ('DateField','DateTimeField'):
                ...
                choices = [
                    ('', '---------'),
                    (today_obj.strftime("%Y-%m-%d"), 'today'),
                    ((today_obj - timezone.timedelta(days=7)).strftime("%Y-%m-%d"), 'last 7 days'),
                    ((today_obj - timezone.timedelta(days=today_obj.day)).strftime("%Y-%m-%d"), 'this month'),
                    ((today_obj - timezone.timedelta(days=90)).strftime("%Y-%m-%d"), 'last 3 month'),
                    ((today_obj - timezone.timedelta(days=180)).strftime("%Y-%m-%d"), 'last 6 month'),
                    ((today_obj - timezone.timedelta(days=365)).strftime("%Y-%m-%d"), 'last 1 year'),
